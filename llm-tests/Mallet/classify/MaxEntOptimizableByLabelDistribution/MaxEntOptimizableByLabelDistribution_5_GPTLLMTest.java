...
if (paramSize > 1)
assertEquals(newParams[1], retrievedParams[1], 1e-10);
}
...
// if (buffer.length > 0 && buffer[0] != 0.0)
allZero = false;
assertTrue(allZero);
}
...
found = gradient[gradient.length - 1];
...
if (gradient[0] == Double.NEGATIVE_INFINITY)
...
// opt.getValueGradient(gradient);
boolean allZero = true;
// if (gradient.length > 0 && gradient[0] != 0.0)
allZero = false;
assertTrue(allZero);
}
...
badParams[0] = Double.NEGATIVE_INFINITY;
opt.setParameters(badParams);
double[] grad = new double[opt.getNumParameters()];
opt.getValueGradient(grad);
boolean hasInf = false;
if (grad.length > 0 && grad[0] == Double.NEGATIVE_INFINITY)
hasInf = true;
assertFalse(hasInf);
}
...
if (grad.length > 0 && Double.isNaN(grad[0]))
hasNaN = true;
assertFalse(hasNaN);
}
}
