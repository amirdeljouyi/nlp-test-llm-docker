...
Instance instance = new Instance(new FeatureSequence(alphabet, new int[] { 0 }), null, null, null);
InstanceList list = new InstanceList(alphabet, null);
list.addThruPipe(instance);
model.addInstances(list);
...
try (PrintWriter pw = new PrintWriter(new OutputStreamWriter(new GZIPOutputStream(new FileOutputStream(tempFile)), "UTF-8"))) {
...
// callables[0] = new ParallelTopicModel.WorkerCallable(model.getNumTopics(), model.alpha, model.alphaSum, model.beta, new cc.mallet.util.Randoms(), model.getData(), model.getTypeTopicCounts(), model.getTokensPerTopic(), 0, model.getData().size());
