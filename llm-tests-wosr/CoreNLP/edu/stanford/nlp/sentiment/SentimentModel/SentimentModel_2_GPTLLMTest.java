...
@Test
  public void testParamsToVectorAndBack() {
    ...
    SentimentModel copy = SentimentModel.modelFromMatrices(W.copy(), Wcat.copy(), Wt.copy(), new HashMap<>(vectors), options);
...
public void testGetWForNodeUnaryThrowsAssertionError() {
  ...
  Tree unary = treeFactory.newTreeNode("X", java.util.Collections.singletonList(treeFactory.newLeaf("word")));
...
public void testGetBinaryClassificationWithCombineClassificationFalse() {
  ...
  SentimentModel model = new SentimentModel(binaryTransform, binaryTensors, binaryClass, unary, vectors, options);
...
public void testToStringIncludesClassificationData() {
  ...
  SentimentModel model = new SentimentModel(binaryTransform, tensors, binaryClass, unary, vectors, options);
  String dump = model.toString();

  assertTrue(dump.contains("Binary classification matrix") || dump.contains("Binary classification matrices"));
  assertTrue(dump.contains("Unary classification matrix") || dump.contains("Unary classification matrices"));
}
@Test
...
public void testConstructorWithNonSimplifiedModelThrowsForBinaryProductions() {
  ...
  TreeFactory tf = new LabeledScoredTreeFactory();
  Tree tree = tf.newTreeNode("S", java.util.Arrays.asList(tf.newLeaf("word1"), tf.newLeaf("word2")));
...
public void testToStringContainsBinaryAndUnaryInfoIfPresent() {
  ...
  SentimentModel model = new SentimentModel(binaryTransform, tensors, binaryClassification, unary, vectors, options);
