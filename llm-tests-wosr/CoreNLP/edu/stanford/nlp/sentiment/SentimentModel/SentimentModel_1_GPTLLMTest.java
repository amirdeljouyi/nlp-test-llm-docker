...
@Test
  public void testUnaryClassificationRetrieval() {
    ...
    SentimentModel model = new SentimentModel(binaryTransform, binaryTensors, binaryClassification, unaryClassification, wordVectors, options);
...
@Test
  public void testGetWordVectorKnownAndUnknown() {
    ...
    SentimentModel model = SentimentModel.modelFromMatrices(
...
@Test(expected = AssertionError.class)
  public void testGetWForNodeTooManyChildrenThrows() {
    ...
    Tree node = new LabeledScoredTreeFactory().newTreeNode("NP", Arrays.asList(child1, child2, child3));
...
@Test(expected = RuntimeException.class)
  public void testInitRandomWordVectorsThrowsWhenNumHidZero() {
    ...
    Tree sentence = new LabeledScoredTreeFactory().newTreeNode("S", Collections.singletonList(leaf));
...
@Test
  public void testTotalParamSizeWithTensorsAndMatrices() {
    ...
    SentimentModel model = new SentimentModel(binaryTransform, binaryTensors, binaryClassification, unaryClass, wordVectors, options);

    int expectedSize = binaryTransform.get("", "").getNumElements()
...
@Test
  public void testGetClassWForUnaryNodeReturnsUnaryMatrix() {
    ...
    Tree parent = new LabeledScoredTreeFactory().newTreeNode("NP", Collections.singletonList(child));
...
@Test
  public void testUnknownWordWithMixedCaseMapping() {
    ...
    wordVectors.put("*UNK*", vec);
    SentimentModel model = SentimentModel.modelFromMatrices(new SimpleMatrix(3, 7), new SimpleMatrix(2, 4), new SimpleTensor(6, 6, 3), wordVectors, options);
...
@Test
  public void testParamsToVectorIncludesAllParameterTypes() {
    ...
    SentimentModel model = new SentimentModel(binaryTransform, binaryTensors, binaryClass, unaryClass, wordVectors, options);
    ...
    assertEquals(
        ...
        + wt.getNumElements()
        + uc.getNumElements()
        + vec1.getNumElements() + vec2.getNumElements(),
        vector.length
    );
  }
@Test
  public void testGetClassWForUnaryNodeReturnsCorrectClassificationMatrix() {
    ...
    Tree leaf = new LabeledScoredTreeFactory().newLeaf("word");
    Tree unary = new LabeledScoredTreeFactory().newTreeNode("NT", new ArrayList<Tree>(Collections.singletonList(leaf)));
...
@Test(expected = AssertionError.class)
  public void testGetClassWForNodeFailsWithThreeOrMoreChildren() {
    ...
    Tree parent = new LabeledScoredTreeFactory().newTreeNode("X", Arrays.asList(left, center, right));
...
@Test
  public void testRandomTransformMatrixIncludesBiasColumn() {
    ...
    SentimentModel model = SentimentModel.modelFromMatrices(new SimpleMatrix(4, 9), new SimpleMatrix(3, 5), new SimpleTensor(8, 8, 4), wordVectors, options);
...
@Test
  public void testWordVectorMappingPreservesCapitalizationIfDisabled() {
    ...
    SentimentModel model = SentimentModel.modelFromMatrices(new SimpleMatrix(2, 5), new SimpleMatrix(2, 3), new SimpleTensor(4, 4, 2), vectors, options);
...
@Test
  public void testVectorToParamsIgnoresExtraParameters() {
    ...
    SentimentModel model = SentimentModel.modelFromMatrices(new SimpleMatrix(2, 5), new SimpleMatrix(2, 3), new SimpleTensor(4, 4, 2), wordVectors, options);
...
@Test
  public void testGetVocabWordLowercasingMissMatchFallback() {
    ...
    SentimentModel model = SentimentModel.modelFromMatrices(new SimpleMatrix(2, 5), new SimpleMatrix(2, 3), new SimpleTensor(4, 4, 2), map, options);
...
@Test
  public void testModelSerializationAndDeserialization() throws IOException, ClassNotFoundException {
    ...
    SentimentModel original = SentimentModel.modelFromMatrices(
        new SimpleMatrix(2, 5),
        new SimpleMatrix(2, 3),
        new SimpleTensor(4, 4, 2),
        wordVectors,
...
@Test
  public void testIdentityMatrixInitializationMatchesExpected() {
    int hiddenSize = 3;
    SentimentModel model = SentimentModel.modelFromMatrices(
        new SimpleMatrix(hiddenSize, hiddenSize * 2 + 1),
        new SimpleMatrix(2, hiddenSize + 1),
        new SimpleTensor(hiddenSize * 2, hiddenSize * 2, hiddenSize),
        Collections.singletonMap("", new SimpleMatrix(2, hiddenSize + 1)),
        new HashMap<>(),
        new RNNOptions() {{
