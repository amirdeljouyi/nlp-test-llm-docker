...
public void testGetBinaryClassificationWithCombineClassificationTrueAlwaysReturnsUnaryMatrix() {
  ...
  SentimentModel model = SentimentModel.modelFromMatrices(binaryTransform, unaryClass, tensor, wordVectors, options);
...
public void testUnaryClassificationSizeWithMultipleEntries() {
  ...
  SentimentModel model = new SentimentModel(binTf, binTensor, binClass, unaryClassification, wordVectors, options);

  assertEquals(2, model.numUnaryMatrices);
  assertEquals(2 * 2 * (2 + 1) / 2, unaryClassification.size() * (model.unaryClassificationSize / 2));
}
@Test
...
public void testModelFromMatricesWithMultipleWordsMaintainsAllVectors() {
  ...
  SentimentModel model = SentimentModel.modelFromMatrices(transform, classMatrix, tensor, wordVectors, options);
...
public void testParamsToVectorIncludesAllComponents() {
  ...
  SentimentModel model = SentimentModel.modelFromMatrices(
      binaryTransform, classification, tensor, wordVectors, options
  );
