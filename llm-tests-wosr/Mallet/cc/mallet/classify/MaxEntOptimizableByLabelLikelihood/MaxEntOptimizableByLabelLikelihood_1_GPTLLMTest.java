...
@Test
    public void testUseGaussianPriorAffectsValue() {
        ...
        InstanceList trainingSet = new InstanceList(pipe);
        trainingSet.addThruPipe(new ArrayIterator(Arrays.asList(
                new Instance("apple banana", "fruit", "inst1", null))));
...
@Test(expected = UnsupportedOperationException.class)
    public void testUseHyperbolicPriorThrowsInValueGradient() {
        ...
        InstanceList trainingSet = new InstanceList(pipe);
        trainingSet.addThruPipe(new ArrayIterator(Arrays.asList(
                new Instance("banana apple", "fruit", "x1", null))));
...
@Test
    public void testUseNoPriorDisablesBothPriors() {
        ...
        training.addThruPipe(new ArrayIterator(Arrays.asList(
                new Instance("apple", "fruit", "inst", null))));
...
@Test
    public void testMultipleCallsToValueAndGradientYieldSameResults() {
        ...
        InstanceList list = new InstanceList(pipe);
        list.addThruPipe(new ArrayIterator(Arrays.asList(
                new Instance("a b c", "label", "id", null))));
...
@Test
    public void testLabelingBestIndexNaNTriggersAssertion() {
        ...
        Instance instance = new Instance(fv, new Labeling() {
            @Override public int getBestIndex() { return (int) Double.NaN; }
            @Override public LabelAlphabet getLabelAlphabet() { return la; }
            @Override public double value(int i) { return 0; }
            @Override public int indexAtLocation(int loc) { return 0; }
            @Override public int numLocations() { return 0; }
            @Override public boolean labelAtLocationIsInstanceOf(Class<?> cls) { return false; }
